...
session_start();
 if(isset($_POST['submit'])){
	...
	if($file_name = $_FILES['image']['name']!=null){
...
// if everything is ok, try to upload file
 ...
 $sql="UPDATE `events` SET `event_name` = '$ename',event_name1='$ename1',date='$date',time='$time',rules='$protocols',partic_no='$nop',cimage='$file_name' WHERE `events`.`event_id` = 10;";
		...
		$sql="UPDATE `events` SET `event_name` = '$ename',event_name1='$ename1',date='$date',time='$time',rules='$protocols',partic_no='$nop',cimage='$cimg'   WHERE `events`.`event_id` = 10;";
		$result=mysqli_query($con,$sql);

		if(isset($result)){
			header("location:/techmeet-1/admin/editevent.php");
		}
